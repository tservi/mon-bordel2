...
http://www.t-servi.com
 ...
 def Output(self,name='', dest=''):
	...
	if(dest=='I'):
			...
			fwrite(f,self.buffer,len(self.buffer))
			fclose(f)
	elif(dest=='S'):
			#Return as a string
			return self.buffer
 ...
 def __putpages__(self):
	...
	for n in range(1,nb+1):
		...
		if(self.PageLinks.has_key(n)):
			...
			self.__out__(annots+']')
		self.__out__('/Contents '+(self.n+1)+' 0 R>>')
		self.__out__('endobj')
		#Page content
		...
		self.__newobj__()
		self.__out__('<<'+filter+'/Length '+len(p)+'>>')
 ...
 def __putresourcedict__(self):
	...
	for font in self.fonts:
		self.__out__('/F'+font['i']+' '+font['n']+' 0 R')
	self.__out__('>>')
 ...
 def __putinfo__(self):
	self.__out__('/Producer '+self.__textstring__('FPDF-python '.FPDF_VERSION))
	...
	if(self.has_key('keywords')):
		self.__out__('/Keywords '.self.__textstring__(self.keywords))
	if(self.has_key('creator')):
		self.__out__('/Creator '.self.__textstring__(self.creator))
	self.__out__('/CreationDate '+self.__textstring__('D:'+str(date('YmdHis'))))
 ...
 def __putcatalog__(self):
	...
	if(self.ZoomMode=='fullpage'):
		...
		self.__out__('/OpenAction [3 0 R /FitH None]')
	elif(self.ZoomMode=='real'):
		self.__out__('/OpenAction [3 0 R /XYZ None None 1]')
	elif(not isinstance(self.ZoomMode,str)):
		self.__out__('/OpenAction [3 0 R /XYZ None None '+str(self.ZoomMode/100)+']')
 ...
 def __puttrailer__(self):
	self.__out__('/Size '+str(self.n+1))
	self.__out__('/Root '+str(self.n)+' 0 R')
	self.__out__('/Info '+str(self.n-1)+' 0 R')
 ...
 def __enddoc__(self):
	...
	o=len(self.buffer)
	self.__out__('xref')
	self.__out__('0 '+str(self.n+1))
	self.__out__('0000000000 65535 f ')
	for i in range(1,self.n+1):
		self.__out__('%010d 00000 n '%(self.offsets[i]))
	#Trailer
...
#print test.__doc__
#print test.compress
#test.SetTitle('testTitle'.encode("UTF-8").encode("UTF-16"))
#print test.title
#test.SetTitle('testTitle'.encode("UTF-8"),True)
#print test.title.decode("UTF-16")
#test.SetSubject('testSubject'.encode("UTF-16"))
#test.SetAuthor('testAuthor'.encode("UTF-16"))
#test.SetKeywords('testKeyword'.encode("UTF-16"))
#test.SetCreator('testCreator'.encode("UTF-16"))
#test.AliasNbPages()
#test.Open()
#test.Error('ceci est un test!')
test=FPDF()
test.AddPage()
